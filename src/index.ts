...
async function main() {
  ...
  for (const keyword of CONFIG.KEYWORDS) {
    try {
      ...
      if (rawJobs.length === 0) {
        console.log(`ℹ️ No se obtuvieron vacantes para la búsqueda: "${keyword}".`);
        continue;
      }
      ...
      for (const job of rawJobs) {
        ...
        if (!description) {
          console.warn('⚠️ No se pudo extraer la descripción de este trabajo. Saltando...');
          continue;
        }
        ...
        if (evaluatedJob) {
          ...
          console.log(`   - ¿Apta para pasantías?: ${evaluatedJob.isSuitableForInternship ? 'SÍ 👍' : 'NO 👎'}`);
